...
function scheduleNotification(date, title, message) {
  ...
  const scheduledNotifications = JSON.parse(
    localStorage.getItem("memento_scheduled_notifications") || "{}"
  );
  ...
  localStorage.setItem(
    "memento_scheduled_notifications",
...
export function scheduleReminderIfNeeded(aiResponse) {
...
export function showWeeklySummaryNotification() {
  const settings = getSettings();
  ...
  const lastSummaryDate = localStorage.getItem(
    "memento_last_weekly_summary_date"
  );
...
export function showMilestoneNotification() {
  const settings = getSettings();
  ...
  for (const milestone of milestones) {
    if (messageCount === milestone) {
      const notifiedMilestones = JSON.parse(
        localStorage.getItem("memento_notified_milestones") || "[]"
      );
...
export function showInactiveUserNotification() {
  const settings = getSettings();
